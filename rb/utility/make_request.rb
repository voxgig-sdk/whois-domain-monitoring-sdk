...
module WhoisDomainMonitoringUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = WhoisDomainMonitoringResponse.new({})
    ...
    if fetch_err
      ...
      response = WhoisDomainMonitoringResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = WhoisDomainMonitoringResponse.new(fetched)
    else
