...
//    only - a typed WhoisDomainMonitoringContext there would make every ctx
...
final class WhoisDomainMonitoringOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new WhoisDomainMonitoringContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new WhoisDomainMonitoringSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new WhoisDomainMonitoringResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new WhoisDomainMonitoringError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new WhoisDomainMonitoringResponse($clean['response']);
    ...
    public static function err_from_map($m): ?WhoisDomainMonitoringError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new WhoisDomainMonitoringError(is_string($code) ? $code : '', $msg);
    }
