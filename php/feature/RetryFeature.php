...
// WhoisDomainMonitoring SDK retry feature
...
class WhoisDomainMonitoringRetryFeature extends WhoisDomainMonitoringBaseFeature
{
    ...
    public function init(WhoisDomainMonitoringContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (WhoisDomainMonitoringContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(WhoisDomainMonitoringContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(WhoisDomainMonitoringContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
