...
// WhoisDomainMonitoring SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class WhoisDomainMonitoringTestFeature extends WhoisDomainMonitoringBaseFeature
{
    ...
    public function init(WhoisDomainMonitoringContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (WhoisDomainMonitoringContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = WhoisDomainMonitoringParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (WhoisDomainMonitoringContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(WhoisDomainMonitoringContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = WhoisDomainMonitoringParam::call($ctx, $k);
