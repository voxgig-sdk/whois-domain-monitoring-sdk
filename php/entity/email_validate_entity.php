...
class EmailValidateEntity
{
    ...
    {
        if ($args) {
            $this->_data = WhoisDomainMonitoringHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = WhoisDomainMonitoringHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   SDK boundary; throws WhoisDomainMonitoringError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = WhoisDomainMonitoringHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
