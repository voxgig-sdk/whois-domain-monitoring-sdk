...
import (
	...
	omni "github.com/voxgig-sdk/whois-domain-monitoring-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.WhoisDomainMonitoringError); is {
				out := map[string]any{
					"name":    "WhoisDomainMonitoringError",
...
func omniCtx(arg any, client *sdk.WhoisDomainMonitoringSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.WhoisDomainMonitoringSDK); is {
			client = live
