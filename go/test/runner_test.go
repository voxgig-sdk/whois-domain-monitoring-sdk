...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("WHOISDOMAINMONITORING_TEST_LIVE") == "TRUE" ||
		os.Getenv("WHOISDOMAINMONITORING_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("WHOISDOMAINMONITORING_TEST_EXPLAIN"); explain != "" {
