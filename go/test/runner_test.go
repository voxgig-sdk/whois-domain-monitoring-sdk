...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("WHOIS_DOMAIN_MONITORING_TEST_LIVE") == "TRUE" ||
		os.Getenv("WHOIS_DOMAIN_MONITORING_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("WHOIS_DOMAIN_MONITORING_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.WhoisDomainMonitoringSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.WhoisDomainMonitoringSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.WhoisDomainMonitoringError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.WhoisDomainMonitoringSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.WhoisDomainMonitoringError{Msg: msg, Code: code}
}
