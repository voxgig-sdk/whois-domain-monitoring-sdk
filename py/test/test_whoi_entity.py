    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set WHOISDOMAINMONITORING_TEST_WHOI_ENTID JSON to run live")
        client = setup["client"]
...
def _whoi_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "WHOISDOMAINMONITORING_TEST_WHOI_ENTID")
    _idmap_overridden = _entid_env_raw is not None and _entid_env_raw.strip().startswith("{")

    env = runner.env_override({
        "WHOISDOMAINMONITORING_TEST_WHOI_ENTID": idmap,
    ...
    idmap_resolved = helpers.to_map(
        env.get("WHOISDOMAINMONITORING_TEST_WHOI_ENTID"))
