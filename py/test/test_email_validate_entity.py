    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set WHOISDOMAINMONITORING_TEST_EMAIL_VALIDATE_ENTID JSON to run live")
        client = setup["client"]
...
def _email_validate_basic_setup(extra):
    ...
    env = runner.env_override({
        "WHOISDOMAINMONITORING_TEST_EMAIL_VALIDATE_ENTID": idmap,
    ...
    idmap_resolved = helpers.to_map(
        env.get("WHOISDOMAINMONITORING_TEST_EMAIL_VALIDATE_ENTID"))
