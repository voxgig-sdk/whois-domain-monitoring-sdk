# WhoisDomainMonitoring SDK test runner
...
class WhoisDomainMonitoringTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            WhoisDomainMonitoringTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = WhoisDomainMonitoringTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = WhoisDomainMonitoringTestRunner.getenv("WHOIS_DOMAIN_MONITORING_TEST_LIVE")
        override = WhoisDomainMonitoringTestRunner.getenv("WHOIS_DOMAIN_MONITORING_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = WhoisDomainMonitoringTestRunner.getenv(key)
        ...
        explain = WhoisDomainMonitoringTestRunner.getenv("WHOIS_DOMAIN_MONITORING_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["WHOIS_DOMAIN_MONITORING_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if WhoisDomainMonitoringTestRunner._test_control is not None:
            return WhoisDomainMonitoringTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                WhoisDomainMonitoringTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            WhoisDomainMonitoringTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = WhoisDomainMonitoringTestRunner.load_test_control()
...
def load_env_local():
    WhoisDomainMonitoringTestRunner.load_env_local()


def env_override(m):
    return WhoisDomainMonitoringTestRunner.env_override(m)


def entity_data(v):
    return WhoisDomainMonitoringTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return WhoisDomainMonitoringTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return WhoisDomainMonitoringTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return WhoisDomainMonitoringTestRunner.load_test_control()


def live_client_options():
    return WhoisDomainMonitoringTestRunner.live_client_options()


def live_delay_ms():
    return WhoisDomainMonitoringTestRunner.live_delay_ms()
