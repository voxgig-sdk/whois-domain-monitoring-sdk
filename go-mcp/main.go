// Command WhoisDomainMonitoring-mcp is an MCP server exposing the WhoisDomainMonitoring
// SDK as MCP tools.
//
// Two tools are registered:
//
//	WhoisDomainMonitoring_list  — list records of an entity
//	WhoisDomainMonitoring_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: WHOIS_DOMAIN_MONITORING_APIKEY carries the API key and
	// WHOIS_DOMAIN_MONITORING_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("WHOIS_DOMAIN_MONITORING_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("WHOIS_DOMAIN_MONITORING_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewWhoisDomainMonitoringSDK(opts)
