# whois-domain-monitoring-mcp

[MCP](https://modelcontextprotocol.io) server exposing the WhoisDomainMonitoring SDK as
two agent tools — `whois-domain-monitoring_list` and `whois-domain-monitoring_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/whois-domain-monitoring-mcp)
make build
...
claude mcp add --scope user whois-domain-monitoring \
  -- /absolute/path/to/whois-domain-monitoring-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./whois-domain-monitoring-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// whois-domain-monitoring_list: first page of records
{ "entity": "domain" }
{ "entity": "domain", "query": { } }

// whois-domain-monitoring_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/whois-domain-monitoring-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user whois-domain-monitoring \
     -- "$PWD"/dist/*/whois-domain-monitoring-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `whois-domain-monitoring_list` and `whois-domain-monitoring_load` tools now appear
   in new sessions. Ask the agent to *"list domain using whois-domain-monitoring"*
   and it calls `whois-domain-monitoring_list` with `{"entity":"domain"}`.
...
### Call the `whois-domain-monitoring_list` tool
...
### Call the `whois-domain-monitoring_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `whois-domain-monitoring_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `whois-domain-monitoring_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./whois-domain-monitoring-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"whois-domain-monitoring_load","arguments":{"entity":"dns_result","query":{"id":1}}}}'
```
