...
const __1 = require("..");
const SDK_NAME = 'WhoisDomainMonitoringSDK';
...
function isRunnable(code) {
    return (/new\s+WhoisDomainMonitoringSDK\b/.test(code) ||
        /\bWhoisDomainMonitoringSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+WhoisDomainMonitoringSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/WhoisDomainMonitoringSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
