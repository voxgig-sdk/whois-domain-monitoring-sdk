...
module WhoisDomainMonitoringOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if WhoisDomainMonitoringOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = WhoisDomainMonitoringOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| WhoisDomainMonitoringOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || WhoisDomainMonitoringOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = WhoisDomainMonitoringSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = WhoisDomainMonitoringError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { WhoisDomainMonitoringOmni.errify(err) }
