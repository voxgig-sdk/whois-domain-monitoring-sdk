...
func NewContext(ctxmap map[string]any, basectx *Context) *Context {
	...
	if c := getCtxProp(ctxmap, "client"); c != nil {
		if sdk, ok := c.(*WhoisDomainMonitoringSDK); ok {
			ctx.Client = sdk
		}
	}
...
func (ctx *Context) MakeError(code string, msg string) *WhoisDomainMonitoringError {
	return NewWhoisDomainMonitoringError(code, msg, ctx)
}
